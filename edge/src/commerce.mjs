...
function normalizedSupply(state) {
  ...
  assert(Number.isSafeInteger(remaining) && remaining >= 0 && remaining <= 10_000, 503, "invalid_supply_state", "Onchain remaining supply is invalid");
  return {
    remaining,
    totalMinted: String(state.totalMinted),
    termsHash: state.termsHash,
    termsUri: state.termsUri,
    blockNumber: String(state.blockNumber),
    mintPriceAtomic: String(state.mintPriceAtomic),
    paymentToken: state.paymentToken,
    paymentTokenDecimals: Number(state.paymentTokenDecimals),
    salesPaused: state.salesPaused === true,
  };
}
...
export function calculateQuote(input, config, chain, supplyState) {
  ...
  const maxKeys = supply.salesPaused
    ? 0
    : Math.min(supply.remaining, 1000, Math.floor(budget / 5000), Math.ceil(forecastCalls / 10_000) + 1);
  ...
  if (hasWallet) {
    if (!isAddress(input.wallet)) throw new EdgeError(400, "invalid_wallet", "wallet must be an EVM address");
    wallet = getAddress(input.wallet);
  }
  const nextAction = actionFor(recommendation, hasWallet, authority);
  const unsignedTransactions = recommendation.startsWith("BUY_") && wallet
    ? chain.purchasePlanTransactions(wallet, best.keys)
    : [];
  const quoteCreatedAt = new Date();
  const reasonCodes = [
    ...(supply.salesPaused ? ["primary_sales_paused"] : []),
    ...(!supply.salesPaused && supply.remaining === 0 ? ["primary_supply_exhausted"] : []),
  ...
  return {
    ...
    quote_valid_until: new Date(quoteCreatedAt.getTime() + 5 * 60_000).toISOString(),
    recommendation,
    reason_codes: [...new Set(reasonCodes)],
    assumptions: [
      "Each eligible call costs 0.01 USDC on paygo.",
      "Each GoldKey costs 50 USDC and includes 10,000 calls for one 365-day term.",
      "Network gas and switching costs are excluded unless supplied.",
    ],
    ...
    supply_remaining: supply.remaining,
    ...
    payment_token: supply.paymentToken,
...
export function renderCommerceResponse(quote) {
  if (quote.key_count > 0 && quote.recommendation.startsWith("BUY_")) {
    const overflow = quote.overflow_paygo_calls > 0
      ? ` Remaining overflow is ${quote.overflow_paygo_calls.toLocaleString("en-US")} paygo calls costing ${quote.overflow_paygo_cost_usdc} USDC, for ${quote.optimized_total_cost_usdc} USDC total.`
      : "";
...
export function calculateRenewalQuote(input, config, chain, pass) {
  ...
  const recommendation = forecastCalls === 0
    ? "DO_NOT_RENEW"
    : economicallyQualified
    ? (pass.active ? "RENEW_AFTER_EXPIRY" : "RENEW_NOW")
    : rawSavings > 0
    ? "MEASURE_USAGE"
    : "USE_PAYGO";
  const nextAction = recommendation === "RENEW_AFTER_EXPIRY"
    ? "WAIT_UNTIL_EXPIRY"
    ...
    ? "USE_PAYGO"
    : "MEASURE_USAGE";
  const transactions = recommendation === "RENEW_NOW" && wallet
    ? sequenceTransactions(chain.renewalTransactions(pass.tokenId))
    : [];
  const now = new Date();
  return {
